...
namespace Server.Gumps
{
	...
	public class HouseGumpAOS : Gump
	{
        ...
        public void AddButtonLabeled(int x, int y, int buttonID, string message, bool enabled)
        {
            if (enabled)
                AddButton(x, y, 4005, 4007, buttonID, GumpButtonType.Reply, 0);

            AddHtml(x + 35, y, 240, 20, message, false, false);
        }
		...
		public static void ClearCoOwners_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay && house.IsOwner( from ) )
			{
                ...
                from.SendMessage("Todos os Sócios foram removidos");
		...
		public static void ClearFriends_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay && house.IsCoOwner( from ) )
			{
                ...
                from.SendMessage("Todos os Amigos foram removidos");
		...
		public static void ClearBans_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay && house.IsFriend( from ) )
			{
                ...
                from.SendMessage("Todos os Bans foram removidos");
			...
			if ( okay && house.IsFriend( from ) )
			{
				...
				for ( int i = 0; i < list.Count; ++i )
				{
					...
					if ( !house.HasAccess( m ) && house.IsInside( m ) )
					{
                        ...
                        m.SendMessage("Seu acesso a casa foi negado.");
						//m.SendLocalizedMessage( 1060734 ); // Your access to this house has been revoked.
					}
				}

                from.SendMessage("Lista de Acessos a Casa foi limpa");
			...
			if ( okay && house.IsOwner( from ) && !house.HasRentedVendors )
			{
				...
				if ( e != null )
				{
					...
					if ( newHouse != null )
					{
						 ...
						 */
						from.SendGump( new NoticeGump( 1060637, 30720, "Voce realizou a troca de casa. O valor da diferença foi depositado em seu banco.", 32512, 420, 280, null, null ) );
						return;
					}
				}
			}
				...
				case 0:
				{
					switch ( index )
					{
						case 0: // Banish
						{
							if ( m_House.Public )
							{
								from.SendMessage( "Selecione quem voce deseja banir da casa" ); // Target the individual to ban from this house.
						...
						case 2: // Grant Access
						{
							if ( !m_House.Public )
							{
                                from.SendMessage("Selecione a pessoa que voce deseja permitir acesso"); // Target the person you would like to grant access to.
				...
				case 3:
				{
					switch ( index )
					{
						...
						case 1: // Add a Co-Owner
						{
							if ( isOwner )
							{
                                from.SendMessage("Selecione quem voce deseja adicionar como Sócio.");
						...
						case 5: // Add a Friend
						{
							if ( isCoOwner )
							{
                                from.SendMessage("Selecione quem voce deseja adicionar como Amigo.");
						...
						case 13: // Make Public
						{
							if ( isOwner )
							{
								...
								if ( BaseHouse.NewVendorSystem )
								{
									...
									from.SendGump( new NoticeGump( 1060637, 30720, "Esta casa agora é publica. Ela ficará sempre aberta e o dono da casa poderá colocar vendedores dentro dela.", 32512, 320, 180, new NoticeGumpCallback( PublicPrivateNotice_Callback ), m_House ) );
								}
								else
								{
                                    from.SendGump(new NoticeGump(1060637, 30720, "Esta casa agora é publica. Ela ficará sempre aberta e o dono da casa poderá colocar vendedores dentro dela.", 0xF8C000, 320, 180, new NoticeGumpCallback(PublicPrivateNotice_Callback), m_House));
								}
				...
				case 5:
				{
					switch ( index )
					{
                        ...
                        case 10:
                        {
                            if (isOwner)
                            {
                                from.SendMessage("Selecione o Cofre que deseja tornar seguro"); // Choose the item you wish to secure
                                from.Target = new SecureTarget(false, m_House);
                            }
                            else
                            {
                                from.SendMessage("Voce deve ser o dono para fazer isto"); // You must be in your house to do this.
                            }
                            break;
                        }
                        case 11:
                        {
                            if (isOwner)
                            {
                                from.SendMessage("Selecione o Cofre que deseja soltar"); // Choose the item you wish to unsecure
                        ...
                        case 12:
                        {
				            if ( isCoOwner )
				            {
                                from.SendMessage("Selecione o item que deseja trancar"); // Lock what down?
					            from.Target = new LockdownTarget( false, m_House );
				            }
				            else if ( isFriend )
				            {
                                from.SendMessage("Voce deve ser o dono ou socio para fazer isto"); // You are not a co-owner of this house.
				            }
				            else
				            {
                                from.SendMessage("Voce deve ser o dono ou socio para fazer isto"); // You must be in your house to do this.
				            }
                            break;
                        }
                        case 13:
                        {
                            if ( isCoOwner )
				            {
                                from.SendMessage("Selecione o item que deseja soltar"); // Choose the item you wish to release
