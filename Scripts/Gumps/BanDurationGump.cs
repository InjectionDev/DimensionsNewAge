...
namespace Server.Gumps
{
	public class BanDurationGump : Gump
	{
		...
		public BanDurationGump( ArrayList list ) : base( (640 - 500) / 2, (480 - 305) / 2 )
		{
			...
			AddHtml( 10, 10, width - 20, 20, "<CENTER>Duração do Ban</CENTER>", false, false );

			//AddImageTiled( 10, 40, width - 20, height - 50, 2624 );
			//AddAlphaRegion( 10, 40, width - 20, height - 50 );

			AddButtonLabeled( 15, 45, 1, "Infinito" );
			AddButtonLabeled( 15, 65, 2, "From D:H:M:S" );

			AddInput( 3, 0, "Dias" );
			AddInput( 4, 1, "Horas" );
			AddInput( 5, 2, "Minutos" );
			AddInput( 6, 3, "Segundos" );

			AddHtml( 170, 45, 240, 20, "Comentários:", false, false );
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				case 0:
				{
					...
					from.SendMessage( "Sem duração definida." );
					return;
				}
				case 1: // infinite
				{
			...
			if ( shouldSet ) {
				...
				for ( int i = 0; i < m_List.Count; ++i )
				{
					...
					if ( comment != null )
						a.Comments.Add( new AccountComment( from.RawName, String.Format( "Duração: {0}, Coemntario: {1}", (( duration == TimeSpan.MaxValue )? "Infinito" : duration.ToString()), comment ) ) );
				}

				if ( duration == TimeSpan.MaxValue )
					from.SendMessage( "Duração do Ban: Infinito" );
				else
                    from.SendMessage("Duração do Ban: {0}", duration);
			}
			else
			{
