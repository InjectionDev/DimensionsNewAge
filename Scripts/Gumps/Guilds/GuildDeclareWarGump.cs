...
namespace Server.Gumps
{
	public class GuildDeclareWarGump : GuildListGump
	{
		...
		{
            AddHtml(20, 10, 400, 35, "Selecione a Guilda que você deseja declarar Guerra", false, false); // Select the guild you wish to declare war on.

			AddButton( 20, 400, 4005, 4007, 1, GumpButtonType.Reply, 0 );
            AddHtml(55, 400, 245, 30, "Enviar Convite!", false, false); // Send the challenge!
				...
				if ( switches.Length > 0 )
				{
					...
					if ( index >= 0 && index < m_List.Count )
					{
						...
						if ( g != null )
						{
							if ( g == m_Guild )
							{
								m_Mobile.SendMessage( "Voce nao pode declarar guerra contra voce mesmo" ); // You cannot declare war against yourself!
							}
							else if ( (g.WarInvitations.Contains( m_Guild ) && m_Guild.WarDeclarations.Contains( g )) || m_Guild.IsWar( g ) )
							{
                                m_Mobile.SendMessage("Voce ja esta em guerra com esta guilda"); // You are already at war with that guild.
							}
							else if( Faction.Find( m_Guild.Leader ) != null )
							{
                                m_Mobile.SendMessage("Voce nao pode declarar guerra enquanto esta em uma faccao"); // You cannot declare war while you are in a faction
							}
							else
							{
