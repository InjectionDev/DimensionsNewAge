...
namespace Server.Regions
{
	...
	public class BaseRegion : Region
	{
        ...
        private static Rectangle2D KnightOfBritainArea = new Rectangle2D(1162, 1692, 255, 197);
        ...
        public override bool OnDamage(Mobile m, ref int Damage)
        {
            ...
            {
                if (KnightOfBritainArea.Contains(m.Location) && m.Map == Map.Felucca)
                {
                    ...
                    {
                        if (KnightOfBritainArea.Contains(damager.Location) && damager.Map == Map.Felucca)
                        {
                            ((PlayerMobile)damager).KnightOfBritainPoints += Damage / 2;
                            //((PlayerMobile)damager).SendMessage(1259, string.Format("KnightOfBritain -> +{0} de atividade por dano em ", Damage / 2) + player.RawName);
                        }
                    }
                }
            }
                ...
                if (KnightOfBritainArea.Contains(m.Location) && m.Map == Map.Felucca)
                {
                    ...
                    if (damager != null && damager is PlayerMobile)
                    {
                        if (KnightOfBritainArea.Contains(damager.Location) && damager.Map == Map.Felucca)
                        {
                            ...
                            ((PlayerMobile)damager).SendMessage(1259, "KnightOfBritain -> +1 pela morte de " + player.RawName);
                        }
                    }
                }
            }
        ...
        private static DateTime dtLastMoongateBCast;
		public override void OnEnter(Mobile m)
		{

            if (Region.Find(m.Location, m.Map).Name == "Moongates" && m.AccessLevel <= AccessLevel.Player)
            {
                if (dtLastMoongateBCast == null || DateTime.Now > dtLastMoongateBCast.AddSeconds(10))
                {
                    Region region = Region.Find(m.Location, m.Map);
                    World.Broadcast(876, true, "Foi notada alguma movimentacao pelos Moongates de Britannia...");
                    //if (region != null && !string.IsNullOrEmpty(region.Name))
                    //    World.Broadcast(876, true, string.Format("Provavelmente em {0}...", region.Name));
                    dtLastMoongateBCast = DateTime.Now;
                }
            }
