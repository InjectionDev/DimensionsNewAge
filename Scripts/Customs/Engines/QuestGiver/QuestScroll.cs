...
namespace Server.Items
{
	public class QuestScroll : Item
	{
		...
		private static string[] m_Story1 = new string[]
		{
            ...
            "Rumores dizem que ele está em", "A profecia diz que está em",
            "Alguns dizem ter visto em", "Dizem que você pode encontrá-lo em", "Foi perdido em",
            "Ele foi deixado para trás em", "Ele estava escondido em", "um velho mapa diz que é em", "Um pergaminho descoberto diz que pode ser encontrado em",
            "Um conto de fadas narra em", "um assistente diz que pode ser encontrado em",
            "Muitos alegaram ter visto em", "Eles alegam que foi roubado enquanto eles estavam no", "Alguém deixou cair ao explorar",
            "Uma cigana disse que pode ser encontrado em"
		};
		...
		private static string[] m_Story2 = new string[]
		{
            ...
            "os quer morto", "quer se vingar da morte de sua filha"," quer se vingar da morte de seu filho",
            "quer livrar a terra deles", "esta a dar recompensas para suas mortes", "tem medo de enfrentá-los por conta própria",
            "é a contratação de muitas pessoas para matá-los", "foi ferido por eles e é bastante irritado", "está lutando para a sua extinção",
            "está tentanto eliminá-los para fora", "não vai descansar até que todos eles sejam eliminados", "não os quer mais em nossa terra"
		};
		...
		private static string[] m_Story3 = new string[]
		{
            ...
            "Pela morte de", "Por decreto de", "Pelo desaparecimento de", "Recompensas oferecidas por",
            "Pelo ataque", "Para aterrorizar", "Pela morte do pai de", "Pela morte da mãe de",
            "Pelo assalto", "Ordenado por"
		};
		...
		public QuestScroll( int level ) : base( 5360 )
		{
			...
			if ( level > 0 )
			{
				...
				if ( tName > 90 )
				{
					NItemName = m_Items5[Utility.Random(m_Items5.Length)] + "'s Scroll de " + m_Items6[Utility.Random(m_Items6.Length)] + " " + m_Items7[Utility.Random(m_Items7.Length)];
				}
				else if ( tName > 45 )
				{
			...
			if ( level == 1 )
			{
				...
				NChance = Utility.RandomMinMax( 10, 30 );
				NNeed = Utility.RandomMinMax( 10, 25 );
			}
			...
			else if ( level == 2 )
			{
				...
				NChance = Utility.RandomMinMax( 15, 35 );
				NNeed = Utility.RandomMinMax( 15, 25 );
			}
			...
			else if ( level == 3 )
			{
				...
				NChance = Utility.RandomMinMax( 25, 40 );
				NNeed = Utility.RandomMinMax( 20, 35 );
			}
			...
			else if ( level == 4 )
			{
				...
				NChance = Utility.RandomMinMax( 30, 45 );
				NNeed = Utility.RandomMinMax( 25, 35 );
			}
			...
			else if ( level == 5 )
			{
				...
				NChance = Utility.RandomMinMax( 35, 60 );
				NNeed = Utility.RandomMinMax( 25, 35 );
			}
			...
			else if ( level == 6 )
			{
				...
				NChance = Utility.RandomMinMax( 45, 70 );
                NNeed = Utility.RandomMinMax(15, 30);
			}

			if ( NType == 1 )
			{
				Name = string.Format("Elimine '{0}' ({1} de {2})", NMonsterType, NGot.ToString(), NNeed.ToString());
			...
			{
				NNeed = 1;
				Name = string.Format("Procurar pelo item '{0}'", NItemName);

				NStory = "Dica: " + m_Story1[Utility.Random(m_Story1.Length)] + " " + NLocation;
			}
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				from.SendMessage( "Esta Quest ja esta completa!" );
			}
			else if ( NType == 1 )
			{
				from.SendMessage( "Selecione o corpo que voce deseja receber os creditos pela morte." );
				from.SendMessage( "O corpo ira sumir apos receber os creditos." );
				from.Target = new CorpseTarget( this );
			}
			else
			{
				from.SendMessage( "Selecione o corpo que voce seja procurar." );
				from.SendMessage( "O corpo ira sumir apos ser procurado." );
				from.Target = new CorpseTarget( this );
			}
		}
		...
		private class CorpseTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				{
					from.SendMessage( "Isto nao eh um corpo." ); // That is not a corpse!
				}
				else if ( !m_Quest.IsChildOf( from.Backpack ) )
				{
                    from.SendMessage("Isto precisa estar em sua Backpack para ser utilizado."); // That must be in your pack for you to use it.
				}
				else
				{
					object obj = targeted;
					...
					if (
					( m_Quest.NLocation == "the Hedge Maze" && from.Map == Map.Felucca && from.X >= 1032 && from.Y >= 2159 && from.X <= 1256 && from.Y <= 2304 ) ||
					( m_Quest.NLocation == "Dungeon Destard" && from.Map == Map.Felucca && from.X >= 5121 && from.Y >= 769 && from.X <= 5372 && from.Y <= 1020 ) ||
					( m_Quest.NLocation == "the Britain Cemetery" && from.Map == Map.Felucca && from.X >= 1336 && from.Y >= 1443 && from.X <= 1390 && from.Y <= 1493 ) ||
					( m_Quest.NLocation == "the Britain Cemetery" && from.Map == Map.Felucca && from.X >= 1336 && from.Y >= 1487 && from.X <= 1375 && from.Y <= 1510 ) ||
					( m_Quest.NLocation == "Dungeon Shame (Level 1)" && from.Map == Map.Felucca && from.X >= 5376 && from.Y >= 0 && from.X <= 5503 && from.Y <= 127 ) ||
					( m_Quest.NLocation == "Dungeon Shame (Level 2)" && from.Map == Map.Felucca && from.X >= 5505 && from.Y >= 0 && from.X <= 5363 && from.Y <= 127 ) ||
					( m_Quest.NLocation == "Dungeon Shame (Level 3)" && from.Map == Map.Felucca && from.X >= 5376 && from.Y >= 131 && from.X <= 5630 && from.Y <= 256 ) ||
					( m_Quest.NLocation == "Dungeon Shame (Level 5)" && from.Map == Map.Felucca && from.X >= 5636 && from.Y >= 0 && from.X <= 5893 && from.Y <= 126 ) ||
					( m_Quest.NLocation == "Dungeon Shame Mage Towers" && from.Map == Map.Felucca && from.X >= 5431 && from.Y >= 175 && from.X <= 5455 && from.Y <= 199 ) ||
					( m_Quest.NLocation == "Dungeon Shame Mage Towers" && from.Map == Map.Felucca && from.X >= 5567 && from.Y >= 183 && from.X <= 5591 && from.Y <= 207 ) ||
					( m_Quest.NLocation == "the Ice Dungeon" && from.Map == Map.Felucca && from.X >= 5663 && from.Y >= 128 && from.X <= 5892 && from.Y <= 263 ) ||
					( m_Quest.NLocation == "the Ice Demon Lair" && from.Map == Map.Felucca && from.X >= 5650 && from.Y >= 319 && from.X <= 5774 && from.Y <= 370 ) ||
					( m_Quest.NLocation == "the Ratman Fort" && from.Map == Map.Felucca && from.X >= 5795 && from.Y >= 313 && from.X <= 5866 && from.Y <= 384 ) ||
					( m_Quest.NLocation == "Dungeon Hythloth (Level 1)" && from.Map == Map.Felucca && from.X >= 5889 && from.Y >= 0 && from.X <= 6005 && from.Y <= 117 ) ||
					( m_Quest.NLocation == "Dungeon Hythloth (Level 2)" && from.Map == Map.Felucca && from.X >= 5905 && from.Y >= 136 && from.X <= 6002 && from.Y <= 244 ) ||
					( m_Quest.NLocation == "Dungeon Hythloth (Level 3)" && from.Map == Map.Felucca && from.X >= 6025 && from.Y >= 134 && from.X <= 6133 && from.Y <= 237 ) ||
					( m_Quest.NLocation == "Dungeon Hythloth (Level 4)" && from.Map == Map.Felucca && from.X >= 6037 && from.Y >= 22 && from.X <= 6125 && from.Y <= 110 ) ||
					( m_Quest.NLocation == "Dungeon Deceit (Level 1)" && from.Map == Map.Felucca && from.X >= 5128 && from.Y >= 521 && from.X <= 5239 && from.Y <= 642 ) ||
					( m_Quest.NLocation == "Dungeon Deceit (Level 2)" && from.Map == Map.Felucca && from.X >= 5270 && from.Y >= 523 && from.X <= 5356 && from.Y <= 636 ) ||
					( m_Quest.NLocation == "Dungeon Deceit (Level 3)" && from.Map == Map.Felucca && from.X >= 5124 && from.Y >= 644 && from.X <= 5233 && from.Y <= 768 ) ||
					( m_Quest.NLocation == "Dungeon Deceit (Level 4)" && from.Map == Map.Felucca && from.X >= 5249 && from.Y >= 640 && from.X <= 5339 && from.Y <= 764 ) ||
					( m_Quest.NLocation == "Dungeon Despise (Level 1)" && from.Map == Map.Felucca && from.X >= 5377 && from.Y >= 511 && from.X <= 5517 && from.Y <= 639 ) ||
					( m_Quest.NLocation == "Dungeon Despise (Level 2)" && from.Map == Map.Felucca && from.X >= 5377 && from.Y >= 640 && from.X <= 5529 && from.Y <= 767 ) ||
					( m_Quest.NLocation == "Dungeon Despise (Level 3)" && from.Map == Map.Felucca && from.X >= 5375 && from.Y >= 768 && from.X <= 5633 && from.Y <= 1022 ) ||
					( m_Quest.NLocation == "Dungeon Wrong (Level 1)" && from.Map == Map.Felucca && from.X >= 5775 && from.Y >= 512 && from.X <= 5892 && from.Y <= 634 ) ||
					( m_Quest.NLocation == "Dungeon Wrong (Level 2)" && from.Map == Map.Felucca && from.X >= 5635 && from.Y >= 506 && from.X <= 5745 && from.Y <= 588 ) ||
					( m_Quest.NLocation == "Dungeon Wrong (Level 3)" && from.Map == Map.Felucca && from.X >= 5682 && from.Y >= 612 && from.X <= 5724 && from.Y <= 673 ) ||
					( m_Quest.NLocation == "Dungeon Khaldun" && from.Map == Map.Felucca && from.X >= 5377 && from.Y >= 1281 && from.X <= 5627 && from.Y <= 1512 ) ||
					( m_Quest.NLocation == "the Trinsic Passage" && from.Map == Map.Felucca && from.X >= 5886 && from.Y >= 1274 && from.X <= 6047 && from.Y <= 1414 ) ||
					( m_Quest.NLocation == "the Fire Dungeon (Level 1)" && from.Map == Map.Felucca && from.X >= 5765 && from.Y >= 1281 && from.X <= 5884 && from.Y <= 1417 ) ||
					( m_Quest.NLocation == "the Fire Dungeon (Level 1)" && from.Map == Map.Felucca && from.X >= 5666 && from.Y >= 1402 && from.X <= 5887 && from.Y <= 1520 ) ||
					( m_Quest.NLocation == "the Fire Dungeon (Level 2)" && from.Map == Map.Felucca && from.X >= 5633 && from.Y >= 1274 && from.X <= 5763 && from.Y <= 1398 ) ||
					( m_Quest.NLocation == "the Fire Dungeon (Level 2)" && from.Map == Map.Felucca && from.X >= 5630 && from.Y >= 1383 && from.X <= 5664 && from.Y <= 1456 ) ||
					( m_Quest.NLocation == "the Sewers" && from.Map == Map.Felucca && from.X >= 6019 && from.Y >= 1418 && from.X <= 6132 && from.Y <= 1520 ) ||
					( m_Quest.NLocation == "Terathan Keep" && from.Map == Map.Felucca && from.X >= 5124 && from.Y >= 1532 && from.X <= 5376 && from.Y <= 1784 ) ||
					( m_Quest.NLocation == "the Solen Hive" && from.Map == Map.Felucca && from.X >= 5639 && from.Y >= 1780 && from.X <= 5934 && from.Y <= 2037 ) ||
					( m_Quest.NLocation == "Dungeon Covetous (Level 1)" && from.Map == Map.Felucca && from.X >= 5373 && from.Y >= 1843 && from.X <= 5508 && from.Y <= 1942 ) ||
					( m_Quest.NLocation == "Dungeon Covetous (Level 2)" && from.Map == Map.Felucca && from.X >= 5374 && from.Y >= 1951 && from.X <= 5622 && from.Y <= 2045 ) ||
					( m_Quest.NLocation == "Dungeon Covetous (Level 3)" && from.Map == Map.Felucca && from.X >= 5533 && from.Y >= 1821 && from.X <= 5630 && from.Y <= 1937 ) ||
					( m_Quest.NLocation == "Dungeon Covetous (Jail Cells)" && from.Map == Map.Felucca && from.X >= 5491 && from.Y >= 1791 && from.X <= 5556 && from.Y <= 1821 ) ||
					( m_Quest.NLocation == "Dungeon Covetous (Lake Cave)" && from.Map == Map.Felucca && from.X >= 5390 && from.Y >= 1780 && from.X <= 5486 && from.Y <= 1838 ) ||
					...
					if ( c.Owner == null )
					{
						if ( m_Quest.NType == 1 )
						{
							from.SendMessage( "Ja eh tarde para isto!" );
						}
						else
						{
							from.SendMessage( "Voce nao achou nada!" );
						}
					...
					if ( obj is Corpse )
					{
						...
						{
							from.SendMessage( "Voce ganhou os creditos por este inimigo e progrediu em sua Quest!" );
							...
							{
								from.PrivateOverheadMessage(MessageType.Regular, 0x44, false, "Quest Completa!", from.NetState);
                                m_Quest.Name = "COMPLETA - Elimine " + m_Quest.NMonsterType + " (" + m_Quest.NGot.ToString() + " de " + m_Quest.NNeed.ToString() + ")";
								m_Quest.Hue = 1258;
							}
							else
							{
								m_Quest.Name = "Elimine " + m_Quest.NMonsterType + " (" + m_Quest.NGot.ToString() + " de " + m_Quest.NNeed.ToString() + ")";
							}
						}
						...
						else if ( ( nSpot == 1 ) && ( m_Quest.NType == 2 ) )
						{
							...
							{
								from.PrivateOverheadMessage(MessageType.Regular, 0x44, false, "Eu achei " + m_Quest.NItemName + "!", from.NetState);
								from.SendMessage( "Voce achou " + m_Quest.NItemName + "!" );
								c.Delete();
								m_Quest.NGot = m_Quest.NGot + 1;
                                m_Quest.Name = "COMPLETA - Procurar " + m_Quest.NItemName;
								m_Quest.Hue = 1258;
							}
							else
							{
								from.SendMessage( "Voce parece estar no local certo, mas ainda nao encontrou o item que procura!" );
								c.Delete();
							}
						}
						else
						{
							from.SendMessage( "Isto nao tem relacao com sua Quest!" );
						}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( NType == 1 )
			{
				Name = sCOMPLETE + "Elimine " + NMonsterType + " (" + NGot.ToString() + " de " + NNeed.ToString() + ")";
			}
			else
			{
				Name = sCOMPLETE + "Procure " + NItemName;
			}
		}
	}
}