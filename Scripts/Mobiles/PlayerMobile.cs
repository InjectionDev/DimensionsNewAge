...
using DimensionsNewAge.Scripts.Customs.Engines.Events.BaseEvent.Util;
...
namespace Server.Mobiles
{
    ...
    public class StatusBackup
    {
        public int Karma { get; set; }
        public int Fame { get; set; }
        public bool DisplayGuildTitle { get; set; }
    }
	...
	public class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
        ...
        public string LastGenericTextPromp { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int KnightOfBritainPoints { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int KnightOfBritainKills { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int SurvivorKills { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int SurvivorWins { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int TheHuntKills { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int TheHuntKillsWins { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int FFAKills { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int FFAWins { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int BloodWarKills { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int BloodWarWins { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int HellGatePoints { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public int HellGateKills { get; set; }

        private StatusBackup m_StatusBackup;
        public StatusBackup StatusBackup
        {
            get
            {
                return m_StatusBackup;
            }
            set
            {
                m_StatusBackup = value;
            }
        }
        ...
        private void SetStatusBackup()
        {
            this.m_StatusBackup = new StatusBackup();

            this.m_StatusBackup.Fame =      this.Fame;
            this.m_StatusBackup.Karma =     this.Karma;
            this.m_StatusBackup.DisplayGuildTitle = this.DisplayGuildTitle;
        }

        public void RestoreStatusBackup()
        {
            if (this.m_StatusBackup == null)
                return;

            this.Fame =         this.m_StatusBackup.Fame;
            this.Karma =        this.m_StatusBackup.Karma;
            this.DisplayGuildTitle = this.m_StatusBackup.DisplayGuildTitle;

            this.m_StatusBackup = null;
        ...
        public void SetIncognitoToEvent()
        {
            this.SetStatusBackup();

            this.NameMod = AntiPanelaUtil.EventRandomName;
            this.Karma = -5000;
            this.Fame = 0;
            //this.HairHue = 0;
            //this.Hue = 0;
            ...
            this.BodyMod = 0x190;
            this.HueMod = 754;
            this.SetHairMods(0x2044, 0x2040);
        ...
        public void DropAllItensToBackpack()
        {
            UndressItem(this, Layer.Arms);
            UndressItem(this, Layer.Bracelet);
            UndressItem(this, Layer.Cloak);
            UndressItem(this, Layer.Earrings);
            UndressItem(this, Layer.Gloves);
            UndressItem(this, Layer.Helm);
            UndressItem(this, Layer.InnerLegs);
            UndressItem(this, Layer.InnerTorso);
            UndressItem(this, Layer.MiddleTorso);
            UndressItem(this, Layer.Neck);
            UndressItem(this, Layer.OneHanded);
            UndressItem(this, Layer.OuterLegs);
            UndressItem(this, Layer.OuterTorso);
            UndressItem(this, Layer.Pants);
            UndressItem(this, Layer.Ring);
            UndressItem(this, Layer.Shirt);
            UndressItem(this, Layer.Shoes);
            UndressItem(this, Layer.Talisman);
            UndressItem(this, Layer.TwoHanded);
		...
		private static void OnLogin( LoginEventArgs e )
		{
            ...
            if (from.Map == Map.TerMur)
                from.MoveToWorld(new Point3D(5142, 1762, 1), Map.Trammel);
            ...
            // Envia convite de evento se algum estiver rolando.
            if (SingletonEvent.Instance.IsEventRunning)
            {
                DimensionsNewAge.Scripts.Customs.Engines.Commands.SendEventInvite(e.Mobile);
                e.Mobile.SendGump(new AlertGump(e.Mobile, "Um Evento está com inscrições abertas neste momento, participe!", "Aviso de Evento"));
            }
		}
		...
		public virtual void RecheckTownProtection()
		{
			...
			if ( isProtected != m_LastProtectedMessage )
			{
                if (isProtected)
                {
                    SendMessage(67, "Voce esta agora sobre protecao dos guardas");
                    //SendLocalizedMessage(500112); // You are now under the protection of the town guards.
                }
                else
                {
                    SendMessage(0x26, "Voce deixou a protecao dos guardas");
		...
		public override bool OnMoveOver( Mobile m )
		{
            ...
            if (m is PlayerMobile && this.Hidden && m.AccessLevel == AccessLevel.Player)
		...
		public override bool OnBeforeDeath()
		{
            ...
            if (Environment.MachineName.ToUpper() == "DANIEL-PC" || this.AccessLevel >= Server.AccessLevel.Developer)
            {
		...
		public override void DoSpeech( string text, int[] keywords, MessageType type, int hue )
		{
			...
			{
                if (Body.BodyID == 987 && text.Length > 1 && text[0] == '=' && AccessLevel >= AccessLevel.GameMaster)
                    text = string.Concat(".", text.Substring(1));
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                case 29:
                    {
                        KnightOfBritainPoints = reader.ReadInt();
                        KnightOfBritainKills = reader.ReadInt();
                        SurvivorKills = reader.ReadInt();
                        SurvivorWins = reader.ReadInt();
                        TheHuntKills = reader.ReadInt();
                        TheHuntKillsWins = reader.ReadInt();
                        FFAKills = reader.ReadInt();
                        FFAWins = reader.ReadInt();
                        BloodWarKills = reader.ReadInt();
                        BloodWarWins = reader.ReadInt();
                        HellGatePoints = reader.ReadInt();
                        HellGateKills = reader.ReadInt();
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            writer.Write(KnightOfBritainPoints);
            writer.Write(KnightOfBritainKills);
            writer.Write(SurvivorKills);
            writer.Write(SurvivorWins);
            writer.Write(TheHuntKills);
            writer.Write(TheHuntKillsWins);
            writer.Write(FFAKills);
            writer.Write(FFAWins);
            writer.Write(BloodWarKills);
            writer.Write(BloodWarWins);
            writer.Write(HellGatePoints);
            writer.Write(HellGateKills);
		...
		{
            if (item is SurvivorStone && this.AccessLevel < Server.AccessLevel.GameMaster )
                return false;
            if (item is SurvivorScheduleStone && this.AccessLevel < Server.AccessLevel.GameMaster)
                return false;
            if (item is TheHuntStone && this.AccessLevel < Server.AccessLevel.GameMaster)
                return false;
            if (item is PremiumSpawner && this.AccessLevel < Server.AccessLevel.GameMaster)
                return false;
