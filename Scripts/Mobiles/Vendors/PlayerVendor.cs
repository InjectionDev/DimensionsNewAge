...
namespace Server.Mobiles
{
	...
	public class PlayerVendor : Mobile
	{
		...
		public void InitBody()
		{
			...
			{
				this.Body = 0x191;
				this.Name = NameList.RandomName( "feminino" );
			}
			else
			{
				this.Body = 0x190;
				this.Name = NameList.RandomName( "masculino" );
			}
		}
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
			if ( !IsOwner( from ) )
			{
				SayTo( from, "So posso aceitar itens do dono do shop." ); // I can only take item from the shop owner.
				return false;
			}
			...
			if ( item is Gold )
			{
				if ( BaseHouse.NewVendorSystem )
				{
					...
					{
                        SayTo(from, "Eu vou pegar isto pelos meus servicos."); // I'll take that to fund my services.
					...
					{
                        from.SendMessage("Seu vendedor possui fundos suficientes para trabalhar, nao pode aceitar este valor."); // Your vendor has sufficient funds for operation and cannot accept this gold.
			...
			else
			{
				...
				{
					SayTo( from, "Nao posso carregar mais." ); // I can't carry any more.
					return false;
				}
			}
		}
		...
		private void OnItemGiven( Mobile from, Item item )
		{
			...
			if ( vi != null )
			{
				...
				from.SendMessage( "Digite o preco e descricao para o item " + name + ". (ESC para nao vender)"); // Type in a price and description for ~1_ITEM~ (ESC=not for sale)
		...
		public override bool CheckNonlocalLift( Mobile from, Item item )
		{
			if ( item.IsChildOf( this.Backpack ) )
			{
				...
				{
					SayTo( from, "Se quiser vender algo e so me dizer!" ); // If you'd like to purchase an item, just ask.
					return false;
				}
			}
		...
		public bool CanInteractWith( Mobile from, bool ownerOnly )
		{
			...
			{
				from.SendMessage( "Voce nao pode comprar aqui pois foi banido desta casa!" ); // You can't shop from this home as you have been banned from this establishment.
				return false;
			}
		...
		public void OpenBackpack( Mobile from )
		{
			if ( this.Backpack != null )
			{
				SayTo( from, "Veja meus itens!" ); // Take a look at my/your goods.
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			{
				vendor.SayTo( from, "Voce e o dono deste shop, pode pegar o que quiser." ); // You own this shop, just take what you want.
				return;
			}
			...
			if ( vi == null )
			{
				vendor.SayTo( from, "Voce nao pode comprar isto." ); // You can't buy that.
			}
		...
		public void CollectGold( Mobile to )
		{
			if ( HoldGold > 0 )
			{
				SayTo( to, "Quantos itens deste tipo: {0} voce gostaria ?", HoldGold.ToString() );
				to.SendMessage( "Digite a quantia de dinheiro que voce deseja coletar (ESC cancelar):" );

				to.Prompt = new CollectGoldPrompt( this );
			}
			else
			{
				SayTo( to, "Nao estou com nenhum gold seu." ); // I am holding no gold for you.
			}
		}
		...
		public int GiveGold( Mobile to, int amount )
		{
			...
			if ( amount > HoldGold )
			{
				SayTo( to, "Desculpe, estou com apenas {0} gold seu.", HoldGold.ToString() );
				return 0;
			}
			...
			if ( amountGiven > 0 )
			{
				to.SendMessage( string.Format("{0} gold foi depositado no seu banco", amountGiven.ToString() )); // ~1_AMOUNT~ gold has been deposited into your bank box.
			}

			if ( amountGiven == 0 )
			{
				SayTo( to, "Seu banco nao pode armazenar esse gold. Vou segurar aqui ate voce resolver.." ); // Your bank box cannot hold the gold you are requesting.  I will keep the gold until you can take it.
			}
			else if ( amount > amountGiven )
			{
				SayTo( to, "Parte do seu banco esta cheio, depositei o possivel."); // I can only give you part of the gold now, as your bank box is too full to hold the full amount.
			}
			else if ( HoldGold > 0 )
			{
				SayTo( to, "Seu gold ja foi transferido." ); // Your gold has been transferred.
			}
			else
			{
				SayTo( to, "Todo o gold foi depositado no banco."); // All the gold I have been carrying for you has been deposited into your bank account.
			}
		...
		public void Dismiss( Mobile from )
		{
			...
			{
				SayTo( from, "Voce nao me pode me despedir enquanto eu estiver segurando suas coisas." ); // You cannot dismiss me while I am holding your goods.
				return;
			}
		...
		{
            from.SendMessage("Digite o nome do vendedor (max 20 caracteres)"); // Enter a new name for your vendor (20 characters max):
		...
		{
            from.SendMessage("Digite o nome do shop (max 20 caracteres)"); // Enter a new name for your shop (20 chars max):
		...
		public bool CheckTeleport( Mobile to )
		{
			...
			if ( Placeholder == null )
			{
				...
				to.SendMessage( "O vendedor foi movido para fora da casa. Ele voltara em 2 minutos.." ); // This vendor has been moved out of the house to your current location temporarily.  The vendor will return home automatically after two minutes have passed once you are done managing its inventory or customizing it.
			}
			else
			{
				Placeholder.RestartTimer();

                to.SendMessage("O vendedor foi movido para fora da casa. Ele voltara em 2 minutos.."); // This vendor is currently temporarily in a location outside its house.  The vendor will return home automatically after two minutes have passed once you are done managing its inventory or customizing it.
			}
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
            ...
            if (e.HasKeyword(0x3C) || e.HasKeyword(0x171) || e.Speech.ToLower().Contains("comprar") ) // && WasNamed(e.Speech))) // vendor buy, *buy*
			{
				if ( IsOwner( from ) )
				{
					SayTo( from, "O Shop e seu, pegue o que quiser." ); // You own this shop, just take what you want.
				}
				else if ( House == null || !House.IsBanned( from ) )
				{
					from.SendMessage( "Selecione o item que seja comprar" ); // Select the item you wish to buy.
					from.Target = new PVBuyTarget();

					e.Handled = true;
				}
			} 
			else if ( e.HasKeyword( 0x3D ) || e.HasKeyword( 0x172 ) || e.Speech.ToLower().Contains("vender") ) // && WasNamed( e.Speech )) ) // vendor browse, *browse
			{
				if ( House != null && House.IsBanned( from ) && !IsOwner( from ) )
				{
					SayTo( from, "Voce nao pode comprar aqui, pois foi banido da casa." ); // You can't shop from this home as you have been banned from this establishment.
				}
				else
				{
            ...
            else if (e.Speech.ToLower().Contains("coletar") || e.HasKeyword(0x3E) || (e.HasKeyword(0x173) && WasNamed(e.Speech))) // vendor collect, *collect
			{
				if ( IsOwner( from ) )
				{
					CollectGold( from );
            ...
            else if (e.Speech.ToLower().Contains("status") || e.HasKeyword(0x3F) || (e.HasKeyword(0x174) && WasNamed(e.Speech))) // vendor status, *status
			{
				if ( IsOwner( from ) )
				{
					SendOwnerGump( from );
            ...
            else if (e.Speech.ToLower().Contains("despedir") || e.HasKeyword(0x40) || (e.HasKeyword(0x175) && WasNamed(e.Speech))) // vendor dismiss, *dismiss
			{
				if ( IsOwner( from ) )
				{
					Dismiss( from );
		...
		private class VendorPricePrompt : Prompt
		{
			...
			private void SetInfo( Mobile from, int price, string description )
			{
				...
				if ( price < 0 ) // Not for sale
				{
					...
					if ( item is Container )
					{
						if ( item is LockableContainer && ((LockableContainer)item).Locked )
							m_Vendor.SayTo( from, "Itens locked nao pode ser vendido." ); // Locked items may not be made not-for-sale.
						else if ( item.Items.Count > 0 )
							m_Vendor.SayTo( from, "Todos os itens no container devem estar para venda." ); // To be not for sale, all items in a container must be for sale.
						else
							setPrice = true;
					}
				...
				else
				{
					...
					{
						price = 100000000;
						from.SendMessage( "Voce nao pode cobrar tao caro por um item... ajustado para 100,000,000 gold." );
					}
		...
		private class CollectGoldPrompt : Prompt
		{
			...
			private void GiveGold( Mobile to, int amount )
			{
				if ( amount <= 0 )
				{
					m_Vendor.SayTo( to, "Tudo bem, vou segurar este dinheiro." );
				}
				else
				{
		...
		private class VendorNamePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				from.SendMessage( "Vendedor mudou de nome" ); // Your vendor has been renamed.
		...
		private class ShopNamePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				if ( !NameVerification.Validate( name, 1, 20, true, true, true, 0, NameVerification.Empty ) )
				{
					m_Vendor.SayTo( from, "Nome nao aceito." );
					return;
				}
