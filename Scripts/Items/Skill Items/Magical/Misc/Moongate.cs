...
namespace Server.Items
{
	...
	public class Moongate : Item
	{
        ...
        private bool m_GateToStarRoom;
        private bool m_JustNewbiePlayer;

        [CommandProperty(AccessLevel.GameMaster)]
        public bool GateToStarRoom
        {
            get
            {
                return m_GateToStarRoom;
            }
            set
            {
                m_GateToStarRoom = value;

                if (m_GateToStarRoom)
                {
                    Name = "Star Room Moongate";
                    Hue = 1570;
                    Light = LightType.Circle300;

                    Point3D starRoomPoint3D = new Point3D(5140, 1761, 5);
                    Target = starRoomPoint3D;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool JustNewbiePlayer
        {
            get { return m_JustNewbiePlayer; }
            set { m_JustNewbiePlayer = value; }
        }
		...
		public virtual void UseGate( Mobile m )
		{
			...
			{
                if (m.Player && m_JustNewbiePlayer)
                {
                    if (((PlayerMobile)m).SkillsTotal >= 3000)
                    {
                        if (m.AccessLevel >= AccessLevel.GameMaster)
                        {
                            m.SendMessage("Voce nao eh newbie, mas tem poder de GM e pode entrar no gate.");
                        }
                        else
                        {
                            m.SendMessage("Este Moongate nao pode ser utilizado por veteranos.");
                            return;
                        }
                    }
                }
			...
			{
				m.SendMessage( "Este Moongate nao tem um destino." );
			}
		}
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            writer.Write(m_JustNewbiePlayer);

            // v2
            writer.Write(m_GateToStarRoom);
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            {
                case 3:
                    m_JustNewbiePlayer = reader.ReadBool();
                    goto case 2;

                case 2:
                    m_GateToStarRoom = reader.ReadBool();
                    break;
            }
	...
	public class MoongateConfirmGump : Gump
	{
		...
		public MoongateConfirmGump( Mobile from, Moongate gate ) : base( Core.AOS ? 110 : 20, Core.AOS ? 100 : 30 )
		{
			...
			if ( Core.AOS )
			{
				...
				AddHtml( 10, 10, 400, 20, "Confirmação de Gate", false, false ); // Gate Warning
                ...
                //else
                AddHtml(10, 40, 400, 200, "Voce tem certeza que deseja entrar no Gate ?", false, true); // Dost thou wish to step into the moongate? Continue to enter the gate, Cancel to stay here
			...
			else
			{
                ...
                AddBackground(0, 0, 420, 400, 9270);
