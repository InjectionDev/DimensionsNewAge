...
namespace Server.Items
{
	...
	public class DyeTub : Item, ISecurable
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is Item )
				{
					...
					if ( item is IDyable && m_Tub.AllowDyables )
					{
                        ...
                        else if (m_Tub is MagicDyeTub && !(item is BaseClothing))
                        {
                            from.SendMessage("Este DyeTub so pode pintar pecas de roupas normais");
                        }
                        else if (((IDyable)item).Dye(from, m_Tub))
                        {
                            from.PlaySound(0x23E);
                            this.MagicDyeTubUpdate();
                        }
					}
            ...
            private void MagicDyeTubUpdate()
            {
                if (m_Tub is MagicDyeTub)
                {
                    MagicDyeTub magicDyeTub = m_Tub as MagicDyeTub;
                    magicDyeTub.QtDyes++;

                    if (magicDyeTub.QtDyes >= magicDyeTub.QtMaxDyes)
                    {
                        magicDyeTub.Hue = 0;
                        magicDyeTub.DyedHue = 0;
                        magicDyeTub.Name = string.Format("Empty Dying Tub");
                        m_from.SendAsciiMessage(0x44, "A tinta especial acabou!");
                    }
                    else
                    {
                        magicDyeTub.Name = string.Format("Magic Dying Tub ({0} cargas)", magicDyeTub.QtMaxDyes - magicDyeTub.QtDyes);
                        m_from.SendAsciiMessage(0x44, string.Format("A tinta especial possui mais {0} cargas!", magicDyeTub.QtMaxDyes - magicDyeTub.QtDyes));
                    }
                }
            }
