			...
			if ( from.InRange( c.Location, 2 ) )
			{
				if ( m_Fruits > 0 )
				{
					...
					else
					{
						...
						from.SendMessage( "Voce pegou algumas frutas e botou na bag" ); // You pick some fruit and put it in your backpack.
					}
				}
				else
                    from.SendMessage("Acabaram as frutas nesta arvore"); // There is no more fruit on this tree
			}
			else
