	...
	public abstract class BaseWand : BaseBashing
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			switch ( m_WandEffect )
			{
                ...
                //Novos Tipos
                case WandEffect.FlameStrike:        list.Add(1060658, "Flame Strike Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.ParalyzeField:      list.Add(1060658, "Paralyze Field Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.EnergyVortex:       list.Add(1060658, "Energy Vortex Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.Explosion:          list.Add(1060658, "Explosion Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.Poison:             list.Add(1060658, "Poison Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.Paralyze:           list.Add(1060658, "Paralyze Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.Resurrection:       list.Add(1060658, "Resurrection Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.Cure:               list.Add(1060658, "Cure Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.ChainLightning:     list.Add(1060658, "Chain Lightning Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.PoisonField:        list.Add(1060658, "Poison Field Cargas\t{0}", m_Charges.ToString()); break;
                case WandEffect.FireField:          list.Add(1060658, "Fire Field Cargas\t{0}", m_Charges.ToString()); break;
		...
		{
            if (Deleted || Charges <= 0 || Parent != from)
				return;

            if (o is StaticTarget || o is LandTarget)
            {
                if ((this is ParalyzeFieldWand || this is EnergyVortexWand || this is ChainLightningWand ||
                    this is PoisonFieldWand || this is FireFieldWand) == false)
                    return;
            }
