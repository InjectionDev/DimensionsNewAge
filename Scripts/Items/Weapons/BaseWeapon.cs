...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
            ...
            list.Add(1060658, "{0}\t{1}", "Dano", string.Format("{0}-{1}", MinDamage.ToString(), MaxDamage.ToString())); // weapon damage ~1_val~ - ~2_val~

            if (Core.ML)
                list.Add(1060659, "{0}\t{1}", "Velocidade", Speed); // weapon speed ~1_val~
            else
                list.Add(1060659, "{0}\t{1}", "Velocidade", Speed);

            if (MaxRange > 1)
                list.Add(1060660, "{0}\t{1}", "Distancia", MaxRange.ToString()); // range ~1_val~

			int strReq = AOS.Scale( StrRequirement, 100 - GetLowerStatReq() );

            if (strReq > 0)
            {
                list.Add("Min Str: {0}", strReq.ToString()); // strength requirement ~1_val~
            ...
            if (m_Hits >= 0 && m_MaxHits > 0)
            {
                list.Add("HP: {0}/{1}", m_Hits, m_MaxHits); // durability ~1_val~ / ~2_val~
