...
namespace Server.Items
{
	...
	public abstract class BaseHouseDoor : BaseDoor, ISecurable
	{
		...
		public bool CheckAccess( Mobile m )
		{
            ...
            if (house == null)
            {
                Console.WriteLine("house == null");
                return false;
            }

            if (!house.IsAosRules)
            {
                Console.WriteLine("!house.IsAosRules");
                return true;
            }

            if (house.Public ? house.IsBanned(m) : !house.HasAccess(m))
            {
                Console.WriteLine("house.Public ? house.IsBanned(m) : !house.HasAccess(m)");
                return false;
            }
		...
		{
			if ( !CheckAccess( from ) )
				from.SendMessage( "Voce nao pode entrar aqui" ); // You are not allowed to access this.
			else
				base.Use( from );
		}
